  ...
  methods: {
    async getUserDetailById() {
      ...
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        ...
        console.log('get', this.$refs.staffPhoto.fileList)
      }
    },
    ...
    async saveUser() {
      ...
      console.log('save1', fileList)

      // 将上传的图片赋值给新定义的fileList,判断该数组中是否有图片尚未上传成功--upload=true）
      if (fileList.some(item => !item.upload)) {
        console.log('sava', fileList)

        this.$message.warning('图片尚未上传成功！')
        return
      }
      //  调用父组件
      // 判断fileList中是否有图片，如果有，按次上传，没有，也允许保存空白，后台接口问题，空白用字符串空格
    ...
    async savePersonal() {
      ...
      await updatePersonal({ ...this.formData, staffPhoto: fileList ? fileList[0].url : ' ' })
      this.$message.success('保存成功')
    }
